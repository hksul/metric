...
def downloadRawData(dType):
  df = fdr.DataReader(, 2000)
  return df
  

def processImage3(dType, partitionType):
...
def processAndInsertToDB(dataType, startDate, endDate):
    ...
    company.insert_one({"index":"image3_%s_%s_%s" % (dataType, startDate.year, endDate.year),"data":data_dict})
    return df_result

def fetchFromDB(dataType, startDate, endDate):

    data_from_db = company.find_one({"index":"image3_%s_%s_%s" % (dataType, startDate.year, endDate.year)})
    df = pd.DataFrame(data_from_db["data"])
    return df

def drawImage3(df_res, dataType, startDate, endDate):
...
if dataType or startY or endY:
  with st.spinner('Processing...'):
    ...
    drawImage3(df_r, dataType, startDate, endDate)
